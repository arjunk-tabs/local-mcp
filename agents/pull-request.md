...
**Important:** After creating the PR, immediately strip any auto-appended footer (e.g. "Made with Cursor") from the PR body. Run `gh pr view --json body -q .body` to read the current body, remove any trailing "Made with Cursor" line (or similar AI-attribution footer), then update with `gh pr edit --body "<cleaned body>"`.
...
- [ ] PR body does not contain "Made with Cursor" or any AI-attribution footer
...
- **No AI-tool footers.** Never include "Made with Cursor", "Generated by AI", or any similar AI-attribution footer in the PR body. If `gh pr create` auto-appends one, immediately edit it out with `gh pr edit`.
